...
(function () {
    ...
    /**
     * Cross-platform URL opener for Adobe Illustrator ExtendScript
     * Opens WhatsApp or external links in the default browser without popup console windows
     * Compatible with Windows and macOS ExtendScript environments
     */
    function openURLInDefaultBrowser(url) {
        ...
        try {
            var isWindows = ($.os.indexOf("Windows") !== -1);
            if (isWindows) {
                // Tier 1: Windows Internet Shortcut (.url) executed via ShellExecute
                try {
                    var shortcutFile = new File(Folder.temp.fsName + "/joynest_wa_" + (new Date().getTime()) + ".url");
                    shortcutFile.open("w");
                    shortcutFile.writeln("[InternetShortcut]");
                    shortcutFile.writeln("URL=" + url);
                    shortcutFile.close();
                    if (shortcutFile.execute()) {
                        return true;
                    }
                } catch (eWin1) {}

                // Tier 2: Silent VBScript
                try {
                    var vbsFile = new File(Folder.temp.fsName + "/joynest_wa_" + (new Date().getTime()) + ".vbs");
                    vbsFile.open("w");
                    vbsFile.writeln('On Error Resume Next');
                    vbsFile.writeln('Set sh = CreateObject("WScript.Shell")');
                    vbsFile.writeln('sh.Run "explorer """ & "' + url.replace(/"/g, '""') + '" & """", 0, False');
                    vbsFile.close();
                    if (vbsFile.execute()) {
                        return true;
                    }
                ...
                // Tier 3: system.callSystem if available
                ...
                // macOS: AppleScript 'open location'
                var appleScriptCode = 'open location "' + url.replace(/\\/g, "\\\\").replace(/"/g, '\\"') + '"';
                var executedMac = false;
                ...
                try {
                    if (typeof ScriptLanguage !== "undefined" && ScriptLanguage.APPLESCRIPT) {
                        app.doScript(appleScriptCode, ScriptLanguage.APPLESCRIPT);
                        executedMac = true;
                    } else if (typeof app.doScript === "function") {
                        app.doScript(appleScriptCode, "AppleScript");
                        executedMac = true;
                    }
                } catch (eMac1) {}

                if (executedMac) return true;

                // Tier 2: AppleScript compiled script file
                try {
                    var scptFile = new File(Folder.temp.fsName + "/joynest_wa_" + (new Date().getTime()) + ".scpt");
                    scptFile.open("w");
                    scptFile.write(appleScriptCode);
                    scptFile.close();
                    if (scptFile.execute()) {
                ...
                // Tier 3: system.callSystem with open command
                if (typeof system !== "undefined" && typeof system.callSystem === "function") {
                    try {
                        system.callSystem('open "' + url.replace(/"/g, '\\"') + '"');
                        return true;
                    } catch (eMac3) {}
                }
            }

            // Universal fallback: temporary HTML redirect launcher
            try {
                var htmlFile = new File(Folder.temp.fsName + "/joynest_wa_" + (new Date().getTime()) + ".html");
                htmlFile.open("w");
                htmlFile.writeln('<!DOCTYPE html><html><head><meta http-equiv="refresh" content="0;url=' + url + '"><script>window.location.href="' + url + '";</script></head><body>Redirecting to WhatsApp...</body></html>');
                htmlFile.close();
                htmlFile.execute();
                return true;
            } catch (eHtml) {}
    ...
    function showLicenseActivationDialog(onSuccess) {
        ...
        actDlg.preferredSize = [480, 545];
        actDlg.size = [480, 545];
        actDlg.minimumSize = [480, 545];
        actDlg.maximumSize = [480, 560];
        ...
        pnlContact.margins = [10, 8, 10, 8];
        pnlContact.spacing = 6;
        pnlContact.add("statictext", undefined, "JoyNest Agency Support: support@joynestagency.com");

        var btnWhatsApp = pnlContact.add("button", undefined, "💬 Get License Key on WhatsApp");
        btnWhatsApp.preferredSize = [270, 32];
        try {
            btnWhatsApp.graphics.font = ScriptUI.newFont(btnWhatsApp.graphics.font.name, "Bold", 11);
        } catch(eFont) {}

        btnWhatsApp.onClick = function() {
            var waUrl = "https://wa.me/8801595306172?text=" + encodeURIComponent("Hello, I want to get a License Key for Automation Suite Pro.");
            openURLInDefaultBrowser(waUrl);
        };
    ...
    function showLicenseDetailsDialog(onDeactivated) {
        ...
        detDlg.preferredSize = [480, 565];
        detDlg.size = [480, 565];
        detDlg.minimumSize = [480, 565];
        detDlg.maximumSize = [480, 580];
        ...
        var btnWhatsAppDet = pnlContact.add("button", undefined, "💬 Get License Key on WhatsApp");
        btnWhatsAppDet.preferredSize = [270, 30];
        try {
            btnWhatsAppDet.graphics.font = ScriptUI.newFont(btnWhatsAppDet.graphics.font.name, "Bold", 11);
        } catch(eFont) {}

        btnWhatsAppDet.onClick = function() {
            var waUrl = "https://wa.me/8801595306172?text=" + encodeURIComponent("Hello, I want to get a License Key for Automation Suite Pro.");
            openURLInDefaultBrowser(waUrl);
        };
    ...
    function showMainWindow() {
        ...
        grpTitleLeft.preferredSize.width = 210;
        ...
        headerSpacer.preferredSize.width = Math.max(6, STACK_WIDTH - 210 - 450);

        var grpHeaderRight = pnlHeader.add("group");
        grpHeaderRight.orientation = "row";
        grpHeaderRight.alignChildren = ["right", "center"];
        grpHeaderRight.spacing = 8;

        var lblSupportText = grpHeaderRight.add("statictext", undefined, "Need help? Contact us on WhatsApp");
        try {
            lblSupportText.graphics.font = ScriptUI.newFont(lblSupportText.graphics.font.name, "Regular", 10);
            lblSupportText.graphics.foregroundColor = lblSupportText.graphics.newPen(lblSupportText.graphics.PenType.SOLID_COLOR, [0.55, 0.60, 0.70, 1], 1);
            lblSupportText.helpTip = "Script-এর যেকোনো সমস্যা, error বা ব্যবহার সংক্রান্ত সাহায্যের জন্য WhatsApp Support-এ যোগাযোগ করুন।";
        } catch (eTxt) {}

        var btnWhatsAppSupport = grpHeaderRight.add("button", undefined, "💬 WhatsApp Support");
        btnWhatsAppSupport.preferredSize = [148, 26];
        btnWhatsAppSupport.helpTip = "Script-এর যেকোনো সমস্যা, error বা ব্যবহার সংক্রান্ত সাহায্যের জন্য WhatsApp Support-এ যোগাযোগ করুন।";
        try {
            btnWhatsAppSupport.graphics.font = ScriptUI.newFont(btnWhatsAppSupport.graphics.font.name, "Bold", 10.5);
        } catch (eW) {}

        btnWhatsAppSupport.onClick = function() {
            var waHelpUrl = "https://wa.me/8801595306172?text=" + encodeURIComponent("Hello, I need help with Automation Suite Pro.");
            openURLInDefaultBrowser(waHelpUrl);
        };

        var btnLicDetails = grpHeaderRight.add("button", undefined, "License Details");
        btnLicDetails.preferredSize = [110, 26];
        try {
